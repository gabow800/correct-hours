...
class XeroReportProcessor:
    ...
    def __init__(self, hours_workbook, rates_workbook):
        self.hours_workbook = hours_workbook
        self.rates_workbook = rates_workbook
        self.original_hours_sheet = self.hours_workbook.active
        self.new_hours_sheet = self.hours_workbook.copy_worksheet(from_worksheet=self.original_hours_sheet)

    def add_new_column_headings(self):
        # new column headings
        self.new_hours_sheet.cell(5, 15, "New total").font = Font(bold=True)
        self.new_hours_sheet.cell(5, 16, "New total per day").font = Font(bold=True)
        self.new_hours_sheet.cell(5, 17, "Days worked").font = Font(bold=True)
        self.new_hours_sheet.cell(5, 18, "Rates").font = Font(bold=True)

    def add_up_hours_of_the_same_day(self, current_date, row_start):
        rows_added_count = 0
        total_hours = 0
        for row in self.new_hours_sheet.iter_rows(min_row=row_start, min_col=1, max_col=14, values_only=True):
            date = row[0]
    ...
    def correct_hours(self, row_start, row_end, overtime):
        time_left = overtime
        for col_idx in range(13, 7, -1):
            for row_idx in range(row_end, row_start - 1, -1):
                value = self.new_hours_sheet.cell(row_idx, col_idx).value
                if value <= 0:
                    continue
                ...
                if corrected_value < 0:
                    ...
                    corrected_value = 0
                    self.new_hours_sheet.cell(row_idx, col_idx, corrected_value).font = Font(bold=True, color="FF0000")
                else:
                    self.new_hours_sheet.cell(row_idx, col_idx, corrected_value).font = Font(bold=True, color="FF0000")
                    return
    ...
    def process_rows(self):
        ...
        for row_idx, row in enumerate(
                self.new_hours_sheet.iter_rows(min_row=1, min_col=1, max_col=14, values_only=True)
        ):
            ...
            if row_number < ROW_OFFSET:
                ...
                continue
            # add new total
            self.new_hours_sheet.cell(row_number, 15, f"=SUM(G{row_number}:M{row_number})")
            # add rate
            date = row[0]
            rate_label = row[3]
            ...
            if total_hours > 38:
                ...
                did_overtime = True
            # new total per day
            new_total_of_hours = self.new_hours_sheet.cell(
